...
void Sim808Manager::logBatteryStatus() {
    char response[50] = "";
    sendATCommand("AT+CBC", 2000);

    if (readSimResponse(2000) == 0) {
        sim808.readBytes(response, sizeof(response) - 1);
        char *pch = strtok(response, " ,");
        int mode = (pch = strtok(NULL, " ,")) ? atoi(pch) : 0;
        int capacity = (pch = strtok(NULL, " ,")) ? atoi(pch) : 0;
        int voltage = (pch = strtok(NULL, " ,")) ? atoi(pch) : 0;

        char logBuffer[100];
        snprintf(logBuffer, sizeof(logBuffer), "Batarya Durumu: Mode: %d, Kapasite: %d%%, Voltaj: %dmV", mode, capacity, voltage);
        LogManager::log(LOG_LVL_INFO, logBuffer);
    }
}
